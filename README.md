...
The current source includes a read-only media-intake installer shell. It can validate supported folders, ISOs, and ISO-containing ZIPs, but installation remains deliberately locked until the remaining patch/no-disc gates are complete. An initial constrained, non-elevating compatibility helper has launched the untouched Black Knight disc executable without mounted media; Vengeance and Mercenaries still need a reproducible SafeDisc 1 transform before the media-only contract is complete.
