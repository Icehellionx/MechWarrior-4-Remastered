...
- This is a newly initialized MechWarrior 4 Remastered workspace derived only in governance shape from the MechWarrior 3 Remastered project. It now has a narrow launcher scaffold, media recognition, manual pipeline, release-tree policy, and rollback-safe Vengeance and Black Knight staging cores; no public installer or qualified runtime baseline exists yet.
...
5. Extend the verified transaction contract to Mercenaries, then connect install/repair/uninstall orchestration to the launcher.
