...
- All three titles use non-elevating, title-specific startup profiles that skip obsolete joystick enumeration and AutoConfig but preserve original cinematics. Vengeance requests a 1024×768 fullscreen surface through the qualified MW3-derived DDrawCompat build with `keepvidmem(1)`. Mercenaries requests the same surface through clean upstream DDrawCompat v0.7.1 with `noactivateapp(0)` because the MW3 fork destabilizes its movie surfaces. Both present as desktop-sized borderless 4:3. Black Knight uses its native 800×600 windowed fallback because its source-built PR1 runtime exits `FEEDFACE` with a `ddraw.dll` proxy. A 30-second launch guard repairs only required resolution/depth keys in the title's actual executable directory if MW4 rewrites its user-owned INI.
