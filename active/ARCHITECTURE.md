...
- `OfficialBlackKnightPr1Transform` stages the complete Vengeance-family plan and applies the exact official PR1 from qualified Mech Pak media. `BlackKnightRuntimeCompatibility` then installs an exact-hash, source-built app-local DLL beside the untouched protected executable. Retail Black Knight uses the same runtime without requiring Mech Pak media. The launcher starts `MW4x.exe` directly; there is no injector, helper process, driver, or launch elevation.
