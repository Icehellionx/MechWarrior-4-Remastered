...
- `LegacyGameRegistration` owns the narrow per-user VirtualStore records required by retail Vengeance and Mercenaries (`CDPath`, `EXE Path`, `Version`, and `FIRSTRUN`). Launch orchestration creates only a matching product record before process start, refuses to replace another installation's record, and unified uninstall removes only a record whose path still matches the owned tree.
