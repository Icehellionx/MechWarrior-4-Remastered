...
- All three titles use non-elevating, title-specific startup profiles that skip obsolete joystick enumeration and AutoConfig. Vengeance and Mercenaries request 1024×768 fullscreen surfaces that the pinned app-local DDrawCompat build presents as desktop-sized borderless 4:3. Black Knight uses its proven native 800×600 windowed fallback because its source-built PR1 SafeDisc runtime exits `FEEDFACE` with either the patched or unmodified `ddraw.dll` proxy, while native exclusive fullscreen raises a hardware error. A 30-second launch guard repairs only required resolution/depth keys in the title's actual executable directory if MW4 rewrites its user-owned INI. Startup movies remain explicitly skipped until their legacy DirectShow surface path works through the wrapper.
