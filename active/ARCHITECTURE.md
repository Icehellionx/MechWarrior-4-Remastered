...
- `LegacyGameRegistration` owns the narrow per-user records required by the retail games (`CDPath`, `EXE Path`, `Version`, and `FIRSTRUN`). Vengeance uses the physical 32-bit HKLM VirtualStore path; the source-transformed Mercenaries runtime and Black Knight use direct 32-bit HKCU product paths. Launch orchestration creates only a matching record before process start, refuses to replace another installation's record, preserves an existing first-run/EULA value, and unified uninstall removes only a record whose path still matches the owned tree.
