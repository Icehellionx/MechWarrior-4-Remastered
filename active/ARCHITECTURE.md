...
- `MW4Remastered.Installer` parses those initial paths, immediately requests inspection through the application service, renders selection state, transactionally reopens/revalidates selected media, and previews a contained destination/free-space plan. Complete qualified Vengeance and Black Knight selections invoke the shared coordinator in dependency order, verify each manifest-owned tree, and hand off to the launcher. Recognized Mercenaries and Mech Pak selections remain explicitly disabled until their transforms/entitlement paths are qualified.
