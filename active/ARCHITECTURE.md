...
- `OfficialBlackKnightPr1Transform` stages the complete Vengeance-family plan, applies the exact official PR1 from qualified Mech Pak media, invokes a zero-argument x86 setup-only capture boundary in owned scratch, and passes the mapped image to `BlackKnightPr1ExecutableTransform`. The managed transform exact-hash gates the protected PR1 input, rebuilds imports/callsites, and admits only exact output SHA-256 `22620369…`; normal launch never loads the capture DLL.
...
- `tools/package/build-release.ps1` publishes self-contained installer/launcher/RTP/capture-host executables, builds the pinned GPL PR1 capture DLL with corresponding source, applies release-tree policy before and after packaging, compiles Inno setup, and emits its SHA-256 sidecar. Setup owns the one deliberate elevation boundary; launcher/game execution remains `asInvoker`. No new package is releasable before exact capture bytes, Defender, full install/uninstall, and dialog-aware runtime qualification pass.
