...
- `MediaSourceSessionFactory` provides transaction-length directory/ISO/ZIP roots and owns every mount and ZIP scratch directory until disposal. Supported single-image Mech Pak ZIPs take a narrower boundary: pinned DiscUtils reads ISO9660/Joliet in memory and `MechPakIsoProjection` writes only the exact resource and official-patch allowlist, never the legacy pack installer, autorun, serial, or DRM payload. `MediaSelectionSessionFactory` reopens a complete selection, re-recognizes every expected layout, and fails closed if media changed after intake.
...
- `OfficialBlackKnightPr1Transform` stages the complete Vengeance-family plan and applies exact official PR1 from qualified Mech Pak media. `BlackKnightPr1ImageCapture` runs the exact source-built capture DLL only inside setup-owned scratch, contains the protected launcher and temporary child in an owned kill-on-close Windows job, and never uses a project injector/helper or broad process-name termination. `BlackKnightPr1ExecutableTransform` reproduces the established PR1 import-descriptor and FirstThunk ordering, repairs exactly 127 qualified branches out of the removed SafeDisc tail, rejects any residual direct executable branch into that tail, declares the five ordinary sections, truncates both SafeDisc-only tails, and exact-hash gates the static output. Only that static `MW4x.exe` enters the manifest; capture DLL/configuration/image are deleted before commit. Retail Black Knight remains unsupported until its separate Vengeance Patch 2 dependency is implemented and qualified.
...
- `LegacyGameConfiguration` owns the mutable legacy INI contract without adding those files to the install manifest. It canonicalizes the complete output to CRLF for the legacy parser. Black Knight receives `optionsx.ini` both beside `MW4X.exe` for bootstrap and at the shared Vengeance runtime root used by its shell scripts; its process working directory is that shared root.
...
- Normal game launch also starts an in-process window lifecycle guard keyed to the exact returned process ID. It waits through MW4's transient 800x600 startup window until dgVoodoo establishes the desktop-covering borderless presentation, then preserves that rectangle, reverses only that window's minimize state without activation or z-order promotion, confines the cursor to its monitor only while it is foreground, and releases confinement on focus loss, process exit, launcher shutdown, or error. The launcher remains alive while guarded games run; a user close hides it until the session ends. This is not an injector, executable transform, topmost policy, or separate launch helper.
...
- All three titles use non-elevating, title-specific startup profiles that skip obsolete joystick enumeration and AutoConfig but preserve original cinematics. The DDrawCompat split remains rollback evidence, not the next release boundary. The current candidate exact-pins stock dgVoodoo2 2.87.5 x86 DirectX DLLs and a common profile with the cursor hook, 30 FPS cap, forced 4:3 content, a centered borderless desktop-sized window, and both watermark paths disabled. This avoids a physical display-mode switch; per-title differences remain allowed when interactive evidence requires them. A 30-second launch guard repairs only required render-resolution/depth keys in the title's actual executable directory if MW4 rewrites its user-owned INI.
- First-launch network authorization is a setup boundary. If multiplayer support needs Windows Defender Firewall rules, setup must request that choice explicitly and create paired exact-executable rules: allow inbound on Private networks and block inbound on Public networks, both with edge traversal disabled. The pair shares a project-owned identity so reinstall and uninstall remove both without touching unrelated policy. The launcher must never leave an active profile undecided and trigger a firewall consent dialog during normal game launch.
