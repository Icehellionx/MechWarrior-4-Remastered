...
- `VengeanceInstallPlanBuilder` owns the current full-install allowlist, 8.3 name restoration, and exact internally prepared executable hash gate. `IVengeanceExecutableTransform` is the only coordinator boundary permitted to produce that executable: callers supply only the two media roots, its output must remain inside coordinator-owned scratch, and the default implementation fails closed until the media-derived transform is qualified.
...
- The Inno package owns the first-run media page: it collects one or more neutral ISO/ZIP paths before Install, requires a selection, and forwards paths only as arguments to `MW4Remastered.Installer`. The package does not inspect, mount, extract, recognize, or retain media itself.
- `MW4Remastered.Installer` parses those initial paths, immediately requests inspection through the application service, renders selection state, transactionally reopens/revalidates selected media, and previews a contained destination/free-space plan. End-user mutation remains disabled until the Vengeance-first dependency-aware orchestration is qualified. The standalone Black Knight proof remains an internal compatibility path, not the product's primary install flow.
