...
- The Black Knight source-built retail loader bundle remains reproducible research evidence but is excluded from the production build and package. Official Black Knight PR1 must be applied to the aggregate scratch tree before any exact PR1 disc-check transform is admitted.
...
- `tools/package/build-release.ps1` publishes self-contained installer/launcher/RTP-host executables, applies release-tree policy before and after packaging, compiles the Inno setup, and emits its SHA-256 sidecar. It does not stage the superseded Black Knight proxy DLL. Setup owns the one deliberate elevation boundary for ISO access and any exact machine-wide original-game registration proven necessary by reference parity; launcher/game execution remains `asInvoker`. No new package is releasable while runtime parity is suspended.
