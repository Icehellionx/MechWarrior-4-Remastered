...
7. Vengeance, Black Knight, and Mercenaries bypass their crashing legacy joystick enumeration and obsolete AutoConfig. Setup now replaces AutoConfig's required INI contract without owning the mutable user file, and the launcher requests 1920×1080 fullscreen. Keyboard/menu launch is proven only for the prior windowed profile; pilot creation, fullscreen stability, controller support, mission entry, and repeated-launch coverage remain open before deciding whether an open-source input/display compatibility layer is warranted.
