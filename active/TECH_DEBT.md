...
18. Black Knight's launch-time app-local DLL and incomplete static v2 are disproven. The revised route uses the exact source-built DLL only during setup-owned capture, contains the SafeDisc process family in an owned kill-on-close job, reproduces the established PR1 import-slot ordering, repairs all 127 tail branches, rejects residual removed-tail targets, removes both loader tails, and installs deterministic static executable `7761c41d…`. Full coordinator cleanup and ownership-safe uninstall now pass; package/Defender, repair, focused intro/menu/Alt-Tab, gameplay, and representative-hardware proof remain mandatory. Never reintroduce `VersionInjector`, a project remote-thread injector, a launch helper, or broad process-name killing; ensure capture DLL/configuration/image cannot enter the installed manifest.
