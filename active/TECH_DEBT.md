...
18. Black Knight's former runtime proxy/helper path is retired. The new PR1 path injects a pinned source-built DLL only inside the setup transaction to capture a mapped image, then installs a deterministic static executable. Hosted capture bytes, current Defender results, complete setup cleanup, and dialog-aware menu/gameplay proof remain mandatory; never route normal launcher/game execution through the capture boundary or stack a renderer wrapper onto it.
19. Official Black Knight PR1 is currently sourced from qualified Inner Sphere/Clan Mech Pak media. Setup therefore rejects a Black Knight selection without at least one pack disc. Find and license a separately distributable official PR1 source, or preserve this dependency explicitly in readiness UI; do not silently fetch an unpinned mirror.
