...
18. Black Knight's pinned SafeDiscLoader2 path is not repeatably stable in the clean packaged layout on current Windows. Its own 32-bit HKCU setup record fixes the incorrect-install gate, but repeated launches can still exit in the protected temporary image or later fail DirectDraw exclusive mode. DDrawCompat v0.7.1 (0BSD, archive SHA-256 `0c33ecb1…`, DLL `f75f0ac4…`) was Defender-clean but caused an immediate exit when combined with the injected protection helper, so it is rejected from release input. Prefer a deterministic media-derived transform or a single integrated source-built compatibility boundary over stacking injected wrappers.
