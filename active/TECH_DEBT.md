...
7. Vengeance, Black Knight, and Mercenaries bypass their crashing legacy joystick enumeration and obsolete AutoConfig. At launch, the configuration owner captures the physical bounds of the monitor containing the foreground launcher, derives the largest fitting 4:3 render surface, and shares that result between the launch arguments and bounded mutable-file guard. A separate exact-PID lifecycle guard enumerates the legacy top-level window instead of depending on a transient `MainWindowHandle`, restores minimized state, pins the presentation rectangle, and confines the cursor only while active. Packaged-launcher gameplay proof for seam removal, non-minimizing Alt-Tab/click return, cursor release/recapture, and multi-monitor selection remains open in all three titles. Black Knight still requires CRLF `optionsx.ini` both beside `MW4X.exe` and at the shared Vengeance runtime root.
...
10. The current automation token is not administrator-elevated even when it can run outside the workspace sandbox. Windows rejected a temporary firewall rule probe. The live C: installation has exact-path Private-allow/Public-block pairs for all three titles, but the F: test intentionally skipped firewall preparation. Source now creates exact executable-scoped Private allow or block according to the setup choice and Public block in either case. Field-prove elevated creation, prompt-free launch for both choices, Private multiplayer when selected, and exact uninstall cleanup. Never disable notifications, broaden to all profiles, or add antivirus exclusions.
