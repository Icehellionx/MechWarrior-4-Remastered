...
8. Startup FMV is black under DDrawCompat with intermittent audio, while the same x86 system already supplies DirectShow, MPEG splitter, MPEG-1 video, and MP2 audio components and the source media probes as ordinary MPEG-PS. `singlemonitor` did not repair playback. Treat this as legacy DirectShow/DDraw surface interoperability rather than a missing-codec assumption; keep `/gosnovideo` as the reversible playable baseline and do not install a system codec pack. In-engine cinematics require a separately qualified wrapper or controlled playback design.
