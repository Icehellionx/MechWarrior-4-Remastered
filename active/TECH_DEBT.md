...
19. Official Black Knight PR1 is sourced only from qualified Mech Pak media. Black Knight without a pack now remains at its supported retail revision and receives the same app-local runtime; setup must clearly report which revision it installed and must never fetch an unpinned mirror.
