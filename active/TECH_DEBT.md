...
9. `0.6.27` proved that a Private-only allow rule does not prevent the normal Windows Defender Firewall listener prompt when the active connection is Public. Source now pairs exact executable-scoped Private allow with Public inbound block rules and exact-name cleanup; field-prove elevated creation, prompt-free launch, multiplayer behavior after switching to Private, and uninstall removal. Never disable notifications, broaden to all profiles, or add antivirus exclusions.
