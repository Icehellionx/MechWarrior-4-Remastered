...
- Every executable's non-system runtime imports must be represented by the title install plan. Runtime smoke must enumerate top-level and child dialog text and fail on EULA, autoconfig, CD requests, `STOP`, `EXCEPTION`, fullscreen errors, or incorrect-install text. Success requires positive identification of the real main menu and, for release qualification, one gameplay path; elapsed responsiveness and an empty log are never sufficient.
- Black Knight's setup-only PR1 capture must normalize at least two captures with different resolved addresses to the same exact output. The capture owner must contain the protected launcher and inherited temporary child in an exact kill-on-close process job, leave no orphan process, and remove its adjacent DLL/configuration before manifest generation. The static transform must reproduce the qualified import descriptor/FirstThunk slot order, repair exactly 127 recorded tail branches, scan all executable code and reject any residual direct `E8`/`E9` target in the removed `0x008A3000..0x008A7FFF` range, retain only five ordinary sections, and prove that no capture DLL, configuration, mapped image, or launch helper enters the installed manifest. A timed responsive process is structural evidence only; focused intro-to-menu, Alt-Tab, pilot creation, and mission load remain mandatory.
