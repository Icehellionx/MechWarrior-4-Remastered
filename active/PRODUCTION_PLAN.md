...
3. [ ] Ship a conservative maximum-quality renderer profile for all three titles: retain a stable 4:3 game surface, use dgVoodoo `Resolution=max` rather than aspect-discarding `desktop`, force 4x MSAA, 16x anisotropic filtering, and 32-bit color, preserve pixel-correct 2D UI/textures, and verify no dgVoodoo watermark appears.
