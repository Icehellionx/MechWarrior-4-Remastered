...
1. [ ] Fix Vengeance's 16:9 opening-movie presentation so its image reaches the left and right edges of a widescreen display without stretching, altering the source movie, cropping gameplay, or changing Black Knight/Mercenaries' correct 4:3 presentation. Global dgVoodoo ROI is rejected; a Defender-clean source-built DDrawCompat movie gate compiled but the final Vengeance executable exited during initialization, and stock dgVoodoo 2.86.5 did not load its official D3D12 sample add-on in a controlled probe. Neither experiment is a release path.
...
3. [ ] Ship a conservative maximum-quality renderer profile for all three titles. `0.6.27` disproved the hard-coded 1024x768 guard plus dgVoodoo `Resolution=max`: it still launched low-resolution and stretched. Source now computes the largest monitor-height 4:3 game surface (`1920x1440` on 2560x1440), keeps the launch argument and guarded INI synchronized, leaves wrapper resolution `unforced`, exposes `max_4_3`, forces 4x MSAA, 16x anisotropic filtering, and 32-bit color, and preserves pixel-correct 2D UI/textures. Visible geometry, detail, and no-watermark proof remain open.
4. [ ] Convert installed Inner Sphere and Clan resource evidence into game-visible Mech Pak activation. Field testing proved the archive flags and one global PID-dialog branch still left chassis unselectable. The exact-hash transform now separately enables the selected-media `ShellCheckClanPak`/`ShellCheckISPak` callbacks in Vengeance, Black Knight, and Mercenaries. Field-prove all eight chassis can actually be selected in Instant Action, multiplayer, and MechLab; do not alter campaign markets.
5. [ ] Match the MW3 remaster's Alt-Tab behavior: presentation remains visible and paused/inactive instead of minimizing, other windows can be layered above it, and clicking or Alt-Tabbing back resumes cleanly in all three titles. `0.6.25` disproved normal-band borderless/fullscreensize. The next candidate uses dgVoodoo's documented fake-fullscreen mode; field-test intro, menus, input capture, pause/inactivity, other-window composition, and return before qualification.
...
Smallest verifiable slice: complete and field-run the unnumbered post-`0.6.25` correction, confirm strict 4:3 outside the separate Vengeance-intro experiment, compare 3D edges against `0.6.22`, and select one Inner Sphere and one Clan chassis in each title.
Focused evidence: no Vengeance-manual bottom stripe; circles/character art remain undistorted; installed profiles report `stretched_ar`, `Resolution=max`, fake fullscreen, 4x MSAA, 16x filtering, and no watermark; all three exact final executables contain the selected pack callback replacements plus the global ownership-success branch; selected chassis enter MechLab or Instant Action without a purchase dialog.
