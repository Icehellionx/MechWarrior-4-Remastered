...
Smoke/regression evidence: static v2 `04fe9acf…` reached omitted SafeDisc target `0x008A3504`; static v3 `7761c41d…` later field-crashed on frame 1 because 110 import operands were misclassified. V4 `b31bd051…` exact-validates those corrections, matches the working structural `.text` except the intentional setup bypass, and stayed correctly titled/responsive for 60 seconds in a production-shaped dgVoodoo tree. A fresh coordinator committed/exact-verified 387 files without capture residue or orphan process. Setup `0.6.21` and two byte-identical 21-file stages passed release policy; setup, stage, and installed tree passed current Defender with zero matching detections. Repair/uninstall and interactive focused intro/menu proof remain open.
