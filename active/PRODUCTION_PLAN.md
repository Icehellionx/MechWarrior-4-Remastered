...
Smoke/regression evidence: static v2 `04fe9acf…` reached omitted SafeDisc target `0x008A3504`; static v3 `7761c41d…` later field-crashed on frame 1 because 110 import operands were misclassified. V4 `b31bd051…` exact-validates those corrections, matches the working structural `.text` except the intentional setup bypass, and reached the real menu/pilot-entry flow in packaged field testing. That run exposed a mixed-CRLF/LF shared-root `optionsx.ini`; setup `0.6.22` repairs both Black Knight locations using full-file CRLF normalization. Its two 21-file stages are byte-identical, release policy passes, and the setup/stage have zero matching current Defender detections. Pilot creation, mission, repair, and uninstall remain open.
