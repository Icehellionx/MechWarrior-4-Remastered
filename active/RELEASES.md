...
- Public release: none.
- Final internal setup `0.6.20` is the first package built from static Black Knight PR1 v3 and the owned process-job cleanup path. It is 172,850,398 bytes with SHA-256 `8c82cd714721a02c753cd486f1cbdc90d5ae228c124779b4014b858bf4fb8dae`; release-tree policy passed. The exact setup and its complete 21-file, 402,118,172-byte unpacked payload produced zero target detections with Defender engine `1.1.26080.3`, intelligence `1.459.256.0`, and real-time protection enabled. It remains internal until focused original-video/menu/Alt-Tab/pilot/mission qualification passes.
...
- Compatibility: Black Knight's launch-time app-local DLL and incomplete static v2 `04fe9acf…` are removed. Source now uses exact hosted GPL DLL `7fbf1fbd…` only during setup-owned capture, contains its protected process family in a kill-on-close job, reproduces the established PR1 IAT ordering and complete 127-branch return map, rejects residual removed-tail targets, and installs deterministic five-section static v3 `7761c41d…`. Two captures produced the same output; a complete 387-file coordinator install exact-verified without capture residue and ownership-safe uninstall preserved an unowned save; a production-shaped launch remained correctly titled/responsive for 60 seconds with videos enabled. Package and Defender gates pass; interactive focused menu/gameplay qualification remains open. The unpatched retail route remains unsupported pending its Vengeance Patch 2 prerequisite.
