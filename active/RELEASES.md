...
- Product source/build: every `/gosnovideo` default is retired. DDrawCompat proved Vengeance and Mercenaries cinematics can render but required incompatible per-title binaries/settings and excluded Black Knight. The 2026-09-17 ecosystem re-audit selects exact-hash dgVoodoo2 2.86.5 as the primary presentation candidate. Setup `0.6.21` is approved only as the next bounded field candidate; public release still requires all three intros, menus, pilot creation, one mission each, and Alt-Tab under that stack.
