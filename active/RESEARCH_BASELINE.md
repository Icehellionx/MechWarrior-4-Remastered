...
The 40,042,724-byte official Patch 3 RTP on Inner Sphere media applies directly to the original retail tree; applying Patch 2 first produces the same Patch 3 outputs except for Patch-2-only `drvmgt.dll`. The patched loader/ICD retain SafeDisc 1.50.20 but use different validated cipher metadata. A new exact-input managed transform produces a deterministic 3,698,688-byte executable with SHA-256 `d0e463d3494581ec6b8402dd77dd558ffb1714fc800306fa45f4e7b58653bfba`, removes the `CdaC14BA.dll` and `ARTPCLNT.dll` import descriptors, and replaces exactly two deferred calls to each legacy client with their existing success result. A clean tree containing the Patch 3 data and both packs' exact resource files opened a responsive `MechWarrior Vengeance` window without mounted media or elevation and passed Defender scanning with the obsolete C-Dilla, ARTP client, and cabarc files absent. This proves the protection-free patched runtime boundary, not yet in-game content visibility. `third_party/VengeancePatch3.lock.json` records the exact user-media inputs and retained outputs.
...
The current Black Knight plan uses the recognized disc plus an exact-hash internal compatibility bundle, not a replacement game executable. It preserves the original `MW4X.EXE`, includes required `DRVMGT.DLL` and the runtime-imported `DSETUP.DLL`, excludes `SECDRV.SYS` and setup executables, and reproduces the setup table's expansion-specific long names without running legacy setup. A fresh 143-file corrected tree exactly verified, and its manifest-owned non-elevating helper plus reproducibly source-built loader opened a responsive `MechWarrior Black Knight` window without mounted media or UAC. Broader runtime behavior remains unqualified.
...
Mercenaries Disc 1 stores 85 `GAME/RESOURCE/...` payload entries in `MSGAME.CAB`; a containment-first extraction reproduced exactly those 85 files (615,038,509 bytes) without running setup. The disc SafeDisc image reports version `50.06.09.3002`. The exact retail loader/ICD/DPLAYER inputs now produce a deterministic 3,895,296-byte executable with SHA-256 `f92bbb8c95a35631ce6f3dfec4c1a9c75306b837f97ce8e1f0323f84fccd1e8e`. The managed transform uses the title's distinct second-layer modifiers and import seeds, repairs protected pointers/imports, removes only the `CdaC14BA.dll` descriptor and uniquely matched title gate, and canonicalizes inert legacy padding. A fresh corrected two-disc coordinator install committed 209 payload files including the runtime-imported `DSETUP.DLL`, excluded C-Dilla, SafeDisc, setup, and Disc 2 crack content, passed exact verification, and opened a responsive `MechWarrior Mercenaries` window without mounted media or UAC. The earlier DirectPlay error did not recur in this fresh tree, and a separate 32-bit COM smoke confirmed the installed DirectPlay object can be created. Broader runtime and official patch behavior remain unqualified.
