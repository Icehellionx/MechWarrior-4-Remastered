...
The current Black Knight experiment uses the recognized expansion disc plus a verified manifest-owned Vengeance base and exact-hash compatibility bundle, not a replacement game executable. It preserves the original `MW4X.EXE`, transforms only the exact EULA export after setup acceptance, excludes `SECDRV.SYS` and setup executables, and reproduces setup-table long names without running legacy setup. A normally imported, reproducibly source-built app-local DLL applies a signature-checked two-call OEP patch. Field testing shows this separate-tree, pre-PR1 result still reaches incorrect-install; the earlier repeated responsive launches were false positives.
