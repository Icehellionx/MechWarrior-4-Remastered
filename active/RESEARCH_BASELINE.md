...
The separate-tree retail Black Knight proxy experiment is superseded. The current path builds the shared Vengeance/`MW4X` tree, applies official PR1 `45.30.04.1908`, uses a pinned GPL DLL only during setup to capture the decrypted mapped image, and statically rebuilds the executable through project-owned managed code. Four distinct captures produced byte-identical final SHA-256 `22620369…`. A bounded debugger run reached DirectSound/DirectInput/DDraw/NVIDIA initialization and stayed responsive beyond 75 seconds without the incorrect-install stop, but main-menu/gameplay proof remains open.
