...
The current Black Knight plan uses the recognized expansion disc plus a verified manifest-owned Vengeance base and exact-hash compatibility bundle, not a replacement game executable. It preserves the original `MW4X.EXE`, transforms only the exact EULA export after setup acceptance, excludes `SECDRV.SYS` and setup executables, and reproduces setup-table long names without running legacy setup. A normally imported, reproducibly source-built app-local DLL applies a signature-checked two-call OEP patch for obsolete installed-media checks. The fresh final tree exactly verified and passed three single-process responsive launches with empty logs, no mounted media, no helper/injector, and no launch UAC. Broader runtime behavior remains unqualified.
