...
| Black Knight PR1, `45.30.04.1908` | Exact official PR1 protected output is `MW4X/MW4x.exe` SHA-256 `be9c1573…` | Apply PR1 in shared scratch, capture at OEP during setup, then emit a static executable; launcher starts it directly | Four mapped captures normalized to identical SHA-256 `22620369…`; the 13-import candidate stayed alive/responding beyond 75 seconds and initialized graphics/audio/input without an incorrect-install stop. This is not menu proof | Hosted capture build, exact Defender/package/full-worker smoke, dialog-aware menu/gameplay/config/save, and representative hardware |
