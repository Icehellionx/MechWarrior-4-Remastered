...
| Black Knight PR1 `45.30.04.1908` | Exact protected output is `MW4X/MW4x.exe` SHA-256 `be9c1573…`; SafeDisc generates per-process trampolines that invalidate a static mapped-image rebuild | Apply Vengeance Patch 3 and Black Knight PR1 from qualified Mech Pak media, keep the exact protected executable, and install a pinned GPL app-local `version.dll`; its OEP hook accepts only the qualified PR1 image and uniquely patches two obsolete setup/media calls | Hosted MSVC DLL `f534b642…` passed Defender. A fresh full coordinator PR1 tree exact-verified, passed Defender, and exposed a responsive `MechWarrior Black Knight` window for 40 seconds with no injector, helper, media, or launch UAC. The unpatched retail route crashed because its required Vengeance Patch 2 state is not yet implemented and is rejected by setup | Replacement PR1-only hosted bytes, packaged setup/repair/uninstall, dialog-aware menu/gameplay/config/save, and representative hardware |
| Mercenaries PR1, `50.07.01.2105` | Exact official PR1 produces a 344,863-byte loader plus 3,915,897-byte `MW4MERCS.ICD` SHA-256 `aafff190…`; C-Dilla/title gates remain | Recognize the exact updater, apply it in contained scratch, then derive the disc-free executable from the qualified PR1 loader/ICD/player set | A real production-worker run consumed the original ISO ZIP plus fix ZIP, applied all 11 official changes, committed an exact 211-file manifest with managed executable SHA-256 `7a806308…`, cleaned scratch/mounts, and passed ownership-safe removal. Runtime is not qualified | Reference registry/file parity, actual menu/gameplay/config/save, no launch UAC, Defender scan, and hardware qualification |
