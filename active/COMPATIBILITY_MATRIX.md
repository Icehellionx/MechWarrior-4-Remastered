...
| Vengeance official Patch 3, `01.30.04.1908` | Exact 40,042,724-byte RTP from Inner Sphere media changes the SafeDisc loader metadata and adds C-Dilla/ARTP imports | Apply the exact-hash official payload through the source-owned non-elevating x86 host in minimal scratch, validate all outputs, retain 11 files, and remove four deferred legacy calls | Patch 3 applies directly to retail without Patch 2. Managed SHA-256 `d0e463d3…`; a fresh 252-file Vengeance plus both-packs tree passed exact verification, Defender, and responsive launch without media/UAC or obsolete files | Prove pack chassis/variants visible in game and exercise packaged UI plus ownership-safe uninstall |
...
| Mercenaries Disc 1, `50.06.09.3002` | SafeDisc 1.50.20 344,863-byte loader plus 3,895,418-byte `MW4MERCS.ICD` and C-Dilla title gate | Deterministically unwrap the exact user-media ICD, repair imports/pointers, remove the C-Dilla descriptor/title gate, and qualify official updates separately | Managed exact-input transform produces deterministic SHA-256 `f92bbb8c…`; a fresh 208-file coordinator payload passed exact verification and Defender, opened a responsive `MechWarrior Mercenaries` window without media/UAC, and uninstalled while preserving an unowned save | Official patch plus broader menu/gameplay/config/save/hardware qualification |
