...
| Vengeance official Patch 3, `01.30.04.1908` | Exact 40,042,724-byte RTP from Inner Sphere media changes the SafeDisc loader metadata and adds C-Dilla/ARTP imports | Apply the official payload to validated user media, then run the exact-input managed transform and omit C-Dilla/ARTP/cabarc additions | Patch 3 applies directly to retail without Patch 2. Managed SHA-256 `25b92158…` opened a responsive Inner Sphere resource tree without media/UAC; obsolete imports and files were absent; Defender found no threats | Replace the proprietary RTP oracle with a product-owned patch application path, then prove pack content visible in game and overlay/uninstall behavior |
