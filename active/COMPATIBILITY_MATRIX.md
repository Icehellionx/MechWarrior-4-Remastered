...
| Black Knight retail / PR1 `45.30.04.1908` | PR1 exact protected output is `MW4X/MW4x.exe` SHA-256 `be9c1573…`; SafeDisc generates per-process trampolines that invalidate a static mapped-image rebuild | Keep the exact hash-gated user-media executable and install a pinned GPL app-local `version.dll`; its OEP hook supports only the two qualified image sizes and uniquely patches the two obsolete setup/media calls | Hosted MSVC DLL SHA-256 `f534b642…` passed Defender. A fresh full coordinator PR1 tree exact-verified, passed Defender, and exposed a responsive `MechWarrior Black Knight` window for 40 seconds with no injector, helper, media, or launch UAC. Retail uses the same runtime but remains to be smoked | Packaged setup/repair/uninstall, dialog-aware menu/gameplay/config/save, retail smoke, and representative hardware |
