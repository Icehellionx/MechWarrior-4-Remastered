...
- Later crash/event evidence disproved the app-local loader, static v2, and static v3 Black Knight routes. V2 retained 32 branches into the removed SafeDisc tail; v3 fixed that map but its classifier still rewired 110 import operands incorrectly, including two entry-point MOV loads, and field testing crashed on frame 1 into stack memory. Static v4 exact-validates those 110 corrections, retains the 127-branch and residual-tail checks, and emits deterministic `MW4x.exe` `b31bd051…` without shipping or requiring the historical comparison executable; normal launch contains no capture DLL or injection path. Package and focused gameplay qualification remain open.
