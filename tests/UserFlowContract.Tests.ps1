...
$graphicsSettings = Get-Content -LiteralPath (Join-Path $root 'src/MW4Remastered.Core/Install/GraphicsSettingsService.cs') -Raw
$graphicsForm = Get-Content -LiteralPath (Join-Path $root 'src/MW4Remastered.Launcher/GraphicsSettingsForm.cs') -Raw
$launcherProgram = Get-Content -LiteralPath (Join-Path $root 'src/MW4Remastered.Launcher/Program.cs') -Raw
$diagnostics = Get-Content -LiteralPath (Join-Path $root 'src/MW4Remastered.Core/Install/InstallationDiagnosticsService.cs') -Raw
...
Assert-True ($launcher -match 'Text\s*=\s*"SETTINGS"' -and $launcher -match 'Text\s*=\s*"DIAGNOSTICS"' -and $launcher -notmatch 'REMOVE GAME FILES') 'Launcher must expose bounded display settings and read-only diagnostics without an unfinished repair action.'
Assert-True ($launcher -match 'TextAlign\s*=\s*ContentAlignment\.MiddleCenter' -and $launcher -match 'TextFormatFlags\.HorizontalCenter') 'Disabled settings buttons must retain centered text in the custom button renderer.'
Assert-True ($launcher -match 'settingsOpening' -and $launcher -match 'dialog\.Shown' -and $launcher -match 'graphicsSettings\.ReadChoices\(statuses\)' -and $launcher -match 'graphicsSettings\.ApplyChoices\(statuses, dialog\.Selection\)' -and $graphicsSettings -match 'OwnedInstallFileReplacementTransaction' -and $graphicsForm -match 'GameDisplayPreset\.Choices' -and $graphicsForm -match 'Centered 4:3 picture' -and $graphicsForm -notmatch 'Pillarboxes:|Gameplay render resolution' -and $graphicsForm -match 'Anisotropic texture filtering' -and $graphicsForm -match 'Antialiasing' -and $launcherProgram -match 'new ActiveMonitorResolutionProvider\(' -and $launcherProgram -notmatch 'ConfiguredGameResolutionProvider' -and $launcher -notmatch 'resolutionSettings') 'The single settings dialog must preview centered 4:3 presentation while launch automatically selects a safe render mode.'
Assert-True ($launcher -match 'diagnostics\.Build\(statusReader\.Read\(\)\)' -and $diagnostics -match 'InstallManifestVerifier' -and $diagnostics -notmatch 'status\.Detail|File\.ReadAllText\(.*[Ll]og') 'Diagnostics must be read-only, verify ownership, and avoid raw status details or logs.'
...
Assert-True ($worker -match '(?s)ValidatePlan\(selection\.Current, plan, alreadyReady\);.*?registration\.ValidateAvailable\(product, planned\.DestinationPath\);.*?sessionFactory\.Open\(selection\.Current\)') 'Setup must reject a live registration conflict before opening media or committing game files.'
Assert-True ($registration -match 'OpenSubKey\(registration\.KeyPath, writable: false\)' -and $registration -match 'RejectLiveConflict\(status, registration, key\)') 'Registration preflight must inspect existing values without modifying them and share the final conflict rule.'
...
Assert-True ($registration -match 'still registered and its executable is present' -and $registration -notmatch 'compatibility registration: \{existingExecutable\}') 'An already installed copy must be explained without exposing its private path.'
Assert-True ($installerProgram -match 'catch \(ExistingGameRegistrationException error\)' -and $installerProgram -match 'return 2;' -and $setup -match 'if ResultCode = 2 then' -and $setup -match 'Remove that installation before installing a second copy') 'The wizard must show an existing-install conflict directly rather than only a generic worker failure.'
