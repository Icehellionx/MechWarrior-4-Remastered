...
Assert-True ($launch -match 'gameConfiguration\.Ensure\(status\)' -and $launch -match 'gameConfigurationGuard\.Protect\(status, processId\)') 'Launch must repair MW4 graphics configuration before start and guard it against the executable startup rewrite.'
Assert-True ($launch -match 'DefaultWidth.*DefaultHeight' -and $launch -match 'productId == "black-knight"' -and $launch -notmatch 'ArgumentList\.Add\("-window"\)') 'All three titles must use the common wrapped fullscreen presentation path; Black Knight must not retain the retired window-only exception.'
