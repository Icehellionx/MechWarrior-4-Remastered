...
Assert-True ($launch -match 'DefaultWidth.*DefaultHeight' -and $launch -match 'productId == "black-knight"' -and $launch -match 'ArgumentList\.Add\("-window"\)') 'Vengeance and Mercenaries must request the borderless legacy surface while Black Knight retains its isolated working fallback.'
