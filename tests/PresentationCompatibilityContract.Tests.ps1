...
Assert-True ($lock.profileSha256 -eq '50f2c43d4f806d238942e643a35c8bf402ac45d251cbf24d6027cbe6ef7e04cf') 'The reviewed MW4 dgVoodoo profile hash must remain recorded.'
...
Assert-True ($profile -match 'OutputAPI\s*=\s*d3d11_fl11_0' -and $profile -match 'ScalingMode\s*=\s*stretched_4_3' -and $profile -match 'FullScreenMode\s*=\s*false' -and $profile -match 'WindowedAttributes\s*=\s*borderless,\s*fullscreensize' -and $profile -match 'AppControlledScreenMode\s*=\s*false') 'The profile must force a normal-band borderless full-screen-size window while preserving MW4 gameplay at 4:3 so Alt-Tab does not minimize it.'
