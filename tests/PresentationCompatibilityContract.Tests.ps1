...
Assert-True ($lock.profileSha256 -eq 'a126cd326ba1e9e96ebedcb7921c535d546933febccf6d7c3b68149a885da90b') 'The reviewed MW4 dgVoodoo profile hash must remain recorded.'
...
Assert-True ($profile -match '(?m)^\s*Resolution\s*=\s*max\s*$' -and $profile -notmatch '(?m)^\s*Resolution\s*=\s*desktop\s*$' -and $profile -match 'Antialiasing\s*=\s*4x' -and $profile -match 'Filtering\s*=\s*16') 'The remaster profile must choose the largest application-aspect render target instead of overriding it with a widescreen desktop target, with 4x MSAA and 16x anisotropic filtering.'
