...
Assert-True ($lock.profileSha256 -eq '72b27b7bbebb7d1a3a3dd136b83f88c79aebcb20ba8edcc20a4703a40c0300a6' -and $plan -match $lock.profileSha256) 'The reviewed MW4 dgVoodoo profile hash must remain recorded and enforced by the install boundary.'
...
Assert-True ($profile -match 'SystemHookFlags\s*=\s*cursor' -and $profile -match 'FPSLimit\s*=\s*30' -and $profile -match 'dgVoodooWatermark\s*=\s*false' -and $profile -match '3DfxWatermark\s*=\s*false') 'The MW4-specific cursor and frame-pacing settings must remain explicit, and every dgVoodoo watermark path must remain disabled.'
