...
Assert-True ($lock.profileSha256 -eq 'fd9413ae24ef4028c19b304b48f7062d53871f25942baaa17d554bfeb1261b93' -and $plan -match $lock.profileSha256) 'The reviewed MW4 dgVoodoo profile hash must remain recorded and enforced by the install boundary.'
...
Assert-True ($profile -match '(?m)^\s*Resolution\s*=\s*unforced\s*$' -and $profile -notmatch '(?m)^\s*Resolution\s*=\s*(desktop|max)\s*$' -and $profile -match 'ExtraEnumeratedResolutions\s*=\s*max_4_3[^\r\n]*1920x1440[^\r\n]*2880x2160' -and $profile -match 'Antialiasing\s*=\s*4x' -and $profile -match 'Filtering\s*=\s*16') 'The remaster profile must preserve the monitor-derived 4:3 game resolution and explicitly expose high-resolution 4:3 fallbacks, with 4x MSAA and 16x anisotropic filtering.'
...
Assert-True ($configuration -match 'ActiveMonitorResolutionProvider' -and $configuration -match 'MonitorFromWindow\(GetForegroundWindow\(\)' -and $configuration -match 'LargestFourByThree' -and $launcher -match 'startInfo\.ArgumentList\.Add\("-f"\)' -and $launcher -match 'resolution\.ToString\(\)') 'Wrapped titles must capture the foreground launcher monitor and request its largest fitting 4:3 physical-pixel surface instead of assuming the primary display or hard-coding a resolution.'
...
Assert-True ($addonPatch -match 'D3DDeviceObjectCreated' -and $addonPatch -match 'IsGameplay3DActive' -and $addonPatch -match 'isGameplay3DActive \? 5\.0f' -and $addonPatch -match 'cropRight = \(float\) iCtx\.srcRect\.right') 'The seam correction must remain Direct3D-gameplay-only, five source pixels, and bottom-right anchored.'
