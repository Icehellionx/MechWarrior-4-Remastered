...
Assert-True (($inno | Select-String -Pattern 'profile=private edge=no protocol=any' -AllMatches).Matches.Count -eq 1 -and $inno -notmatch 'profile=(public|any|all)') 'Setup-created inbound rules must remain limited to private networks without edge traversal.'
...
Assert-True (($inno | Select-String -Pattern 'RemovePrivateFirewallRule\(' -AllMatches).Matches.Count -eq 4 -and $inno -match '(?s)RemovePrivateFirewallRule\(''MechWarrior 4 Remastered - Mercenaries''\);\s*if FirewallPage\.Values\[0\]') 'Setup must remove stale project rules even when a reinstall opts out of multiplayer firewall access.'
...
Assert-True ($build -match 'DgVoodooArchive' -and $build -match '76b6893a0be81e3905a03f30f25202d6dc6128c3b8f7a21f2c33bcfabfa75ddf') 'Package must exact-hash the complete evaluated dgVoodoo2 archive.'
Assert-True ($build -match '62f1e1b2ac5196f4a74b35b898ba8644322f976026324e2f767a4096bfb58748' -and $build -match 'b7401378b2b8e8c18c88a033e77c3ee99f4c7d2ac8cfcc949d79c1dd7fa99767') 'Package must exact-hash the stock x86 DirectX wrapper files.'
...
Assert-True ($build -match '454ef91d0efeec8ea5302b57f72258a96736f923ffb34f7e893e245babdfa18e') 'Package staging must exact-hash the qualified MW4 dgVoodoo profile.'
