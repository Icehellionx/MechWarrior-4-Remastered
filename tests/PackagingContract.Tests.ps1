...
Assert-True ($build -match 'UseExistingRestore' -and $build -match 'UseExistingRestore -and -not \$StageOnly' -and $build -match 'never a setup package') 'Offline restored-package reuse must be explicit and limited to internal stages.'
...
Assert-True ($inno -match '\*\.iso;\*\.zip;\*\.cue' -and $inno -match 'matching BIN') 'Setup must expose qualified CUE/BIN intake and explain the required companion BIN.'
...
Assert-True ($inno -match "if AllowPrivate then\s*PrivateAction := 'allow'\s*else\s*PrivateAction := 'block'" -and $inno -match [regex]::Escape('action='' + PrivateAction + '' program=') -and $inno -match 'profile=private edge=no protocol=any' -and $inno -match 'action=block program=' -and $inno -match 'profile=public edge=no protocol=any' -and $inno -notmatch 'profile=(any|all)') 'Setup must decide inbound access for both profiles before launch, blocking Private too when multiplayer was not selected.'
...
Assert-True (($inno | Select-String -Pattern 'RemoveFirewallRules\(' -AllMatches).Matches.Count -eq 5 -and $inno -match '(?s)RemoveFirewallRules\(''MechWarrior 4 Remastered - Mercenaries''\);\s*FirewallWarning := '''';' -and ($inno | Select-String -Pattern 'FirewallPage\.Values\[0\]\)' -AllMatches).Matches.Count -eq 3) 'Setup must replace stale project rules with a complete rule pair for every installed game, including when multiplayer was declined.'
...
Assert-True ($build -match '7ea9e4576a421157927de2d41551e3fdef8b4c76cd3adcc2d02ef19706f249a4') 'Package staging must exact-hash the reviewed D3D12 MW4 dgVoodoo profile.'
Assert-True ($build -match 'DgVoodooSourceRoot' -and $build -match 'build-dgvoodoo-mw4-addon\.ps1' -and $build -match '24e6fe3e7eea55aa2271223bf08e444597e58580f78126ea440db0e2cefb254b') 'Package staging must build and exact-hash the reproducible MW4 presentation add-on from pinned source.'
