...
public sealed class BlackKnightEulaTransform : IBlackKnightEulaTransform
{
    ...
    private static readonly byte[] OriginalExportPrefix = [0x83, 0xec, 0x0c, 0x8b, 0x15, 0x04];
    private static readonly byte[] AcceptedExport = [0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3];
    ...
    public string Transform(string discRoot, string outputDirectory)
    {
        ...
        if (bytes.Length < ExportFileOffset + OriginalExportPrefix.Length ||
            !bytes.AsSpan(ExportFileOffset, OriginalExportPrefix.Length).SequenceEqual(OriginalExportPrefix))
            throw new InvalidDataException("Black Knight EBUEULA.DLL does not contain the qualified export entry point.");

        // Setup already presents the original license and requires explicit
        // acceptance. Return the module's observed accepted result so the
        // legacy dialog does not interrupt first play.
        AcceptedExport.CopyTo(bytes, ExportFileOffset);
        ...
        if (!outputHash.Equals(OutputSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException($"Black Knight accepted EULA module produced an unexpected SHA-256: {outputHash}");
