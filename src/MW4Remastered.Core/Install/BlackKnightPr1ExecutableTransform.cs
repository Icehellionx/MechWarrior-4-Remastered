...
{
    public const string TransformId = "black-knight-pr1-static-v1";
    public const string InputSha256 = "be9c15731b2ab59471f35add8df4935ea65355cbb4672bc48b63295ad83632b0";
    public const string OutputSha256 = "22620369a1130cda8deca1c3752808e26c26d94fe10c27aed5ae0d463fe2888f";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (root.GetProperty("formatVersion").GetInt32() != 1 ||
            ...
            !root.GetProperty("outputSha256").GetString()!.Equals(OutputSha256, StringComparison.Ordinal))
        ...
        WriteUInt32(protectedBytes, pe.OptionalHeaderOffset + 104, idata.VirtualAddress);
        WriteUInt32(protectedBytes, pe.OptionalHeaderOffset + 108, checked((uint)((importCount + 1) * 20)));
        WriteUInt32(protectedBytes, pe.OptionalHeaderOffset + 192, TargetIatRva);
        WriteUInt32(protectedBytes, pe.OptionalHeaderOffset + 196, TargetIatSize);
        WriteUInt32(protectedBytes, pe.OptionalHeaderOffset + 64, 0);
        WriteUInt32(protectedBytes, pe.RequireSection(".rdata").HeaderOffset + 36, 0xC0000040);
        ...
        var actualHash = Hash(protectedBytes);
        ...
        File.WriteAllBytes(destination, protectedBytes);
        return destination;
    }
    ...
    private static void RetargetCallsites(byte[] bytes, Pe32Image pe, JsonElement callsites, JsonElement imports)
    {
        ...
        if (patched.Count != 2_984) throw new InvalidDataException($"Unexpected Black Knight PR1 callsite count: {patched.Count}.");
    }
    ...
    private static void RestoreTailBranches(byte[] bytes, Pe32Image pe, JsonElement branches, JsonElement imports)
    {
        ...
        if (patched != 95) throw new InvalidDataException($"Unexpected Black Knight PR1 tail-branch count: {patched}.");
    }
    ...
    private sealed class Pe32Image
    {
        ...
        private Pe32Image(int optionalHeaderOffset, IReadOnlyList<PeSection> sections)
        {
        ...
        public static Pe32Image Parse(byte[] bytes)
        {
            ...
            return new Pe32Image(optionalOffset, result);
        }
