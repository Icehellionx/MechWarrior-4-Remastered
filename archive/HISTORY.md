...
- A subsequent Vengeance-intro-only research slice rejected two non-release paths: the Defender-clean MW3-derived DDrawCompat crop built but the final Vengeance executable exited after Direct3D initialization, while the stock dgVoodoo 2.86.5 D3D12 run did not initialize its official sample add-on. Global `DisplayROI=16_9`, private CPL control, injection, and external playback followed by video suppression remain outside the preservation contract. The installed and staged dgVoodoo profiles explicitly disable the watermark; visible absence remains a field gate.
- Field candidate `0.6.26` then compiled twice to the same 172,879,541-byte setup (`3a6e1add…`), passed release-tree policy in both builds, and passed current Defender with real-time protection enabled. It carries the aspect-safe profile, fake-fullscreen trial, no-watermark setting, and selected-media script callback activation; it does not claim to solve the separate Vengeance intro crop.
...
- The first Vengeance manual recrop used a 509.8 x 342-point noise-tail heuristic and was field-rejected because it retained a white strip at the bottom. The superseding reviewed page boundary is 509.8 x 323 points; the 98-page output is deterministic at SHA-256 `0343f468…` and all pages passed render/contact-sheet review.
- Internal `0.6.24` corrected three `0.6.23` field regressions: strict 4:3 output replaced global 16:9 stretching, the Vengeance PDF height was reduced to the reviewed 323-point boundary, and exact final executables gained a selected-media-only bypass of the obsolete post-recognition SafeCast PID gate. A seven-ISO unattended install exact-verified both game families and both packs; setup and installed tree passed current Defender scanning. Interactive aspect, anti-aliasing, chassis selection, and Alt-Tab remain field gates.
- The first real-destination `0.6.24` attempt failed after file verification because an earlier disposable smoke had left three project-owned registrations pointing at its deleted tree. The strict live-install collision check behaved as written but could not distinguish a dead coherent owner, and Inno rollback deleted the worker log named by its error dialog. `0.6.25` narrowly permits replacement only when the old expected executable is absent and its product name, media path, and version remain coherent; live or malformed ownership still fails closed. It also copies a failure log outside `{app}` before rollback. A controlled recurrence completed the full default-path seven-ISO install and current Defender scans.
